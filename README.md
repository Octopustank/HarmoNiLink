<p align="center">
  <img src="pics/app-icon.png" width="110" alt="NiLink 图标"/>
</p>

<h1 align="center">HarmoNiLink</h1>

<p align="center">
  <a href="https://appgallery.huawei.com/app/detail?id=com.octopustank.nilink"><img src="https://img.shields.io/badge/%E4%B8%8B%E8%BD%BD-AppGallery-E60012?logo=huawei&logoColor=white" alt="AppGallery 下载"/></a>
  &nbsp;
  <a href="./LICENSE"><img src="https://img.shields.io/badge/License-MulanPSL_v2-blue.svg" alt="License: MulanPSL v2"/></a>
</p>

<p align="center"><em>Nikon Smart GPS for HarmonyOS NEXT</em></p>

<p align="center">
  NiLink 是一款为原生鸿蒙打造的摄影 GPS 辅助工具。通过蓝牙将手机实时位置持续发送给尼康相机，让拍下的每一张照片自动记录经纬度、海拔等信息，方便后期整理、行程追溯和创作分享。
</p>

> [!IMPORTANT]
> 本应用为独立第三方开源项目，与尼康（Nikon）官方无任何合作、授权或隶属关系。提及的所有商标及型号仅用于说明适配范围，不代表官方背书。

鸿蒙设备在卓易通中运行 SnapBridge 存在功能异常，故开发本应用代替之。
...
### 全自动稳定连接与 GPS 分发
扫描、配对、GPS 传输全流程自动化。用户只需要按照指引进行配对、连接，一切细节、断开重连等等都自动完成。

\* Z 系列与 D 系列均已适配蓝牙时序。实际连接可靠性会受使用环境、尼康相机自身设计等限制。
...
每一台已配对相机都记录最近 2 小时的连接健康度，四级状态直观呈现（正常 / 偶发断连 / 定位异常 / 完全中断），拍摄期间连接稳定性心中有数。
...
\* 由于仅测试了 2 台设备同时连接，在连接更多设备时设置了提示。如需调整连接上限或提示阈值，可在代码中修改后重新构建。

### 相机休眠兼容
尼康相机进入休眠后维持 BLE 低功耗连接，GPS 数据持续写入。若连接意外中断（深度休眠 / 超出距离），自动执行指数退避重连。

\* 相机侧须开启「照相机关闭时发送」才能实现相机休眠唤醒。

### 鸿蒙原生后台保活
基于 `BLUETOOTH_INTERACTION` + `LOCATION` 双长时任务，模式随连接状态动态调整，不依赖无声音频等取巧手段。针对长时任务被系统取消或挂起的极少数情况，回到前台还会自动重新拉起并弹窗说明中断原因。

### 原生鸿蒙体验
- **HDS 设计套件** — 沉浸光感、模糊标题栏、自适应背景材质等等
- **亮/暗主题跟随** — 自动跟随系统色彩模式
...
- **同相机多设备可区分** — 每次安装自动生成唯一配对名称（`HarmoNiLink-XXXX`），写入相机并显示在相机端已配对列表，多台手机同时配对同一台相机时互不混淆；连接页可随时查看本机名称。
- **智能扫描分组** — 扫描时通过蓝牙广播中的服务特征（0xDE00）自动识别尼康相机并置顶分组，手机、耳机、打印机等其他设备收进折叠区——多相机、嘈杂环境一目了然，不再大海捞针。
- **配对冲突指引** — 当相机端正在等待另一台已配对设备时（多台手机配对过同一台相机），App 会提示在相机「已配对设备」中切换当前设备。
- **克制的评分引导** — 仅在应用真正交付价值后（首次成功下发 GPS 满一定时间、且近期连接健康）弹出邀请，并采取间隔策略确保一段时间内不再弹出。
...
| 相机 | 尼康 Z 系列（Z 6II、Z 7II、Z 8、Z 9、Z f 等）与 D 系列（D5600 等支持蓝牙接收定位信息的机型） |
...
│   ├── BleEvent.ets                   — 会话级 BLE 事件定义
│   ├── BleScanner.ets                 — BLE 设备扫描
│   └── protocol/
│       ├── NikonPairingEngine.ets      — 4 阶段配对握手
│       ├── PairingMessage.ets         — 17 字节配对消息编解码
...
│   ├── ConnectionTypes.ets            — 共享连接状态定义（解耦 Hub 与页面的循环依赖）
│   ├── ClassicBonding.ets             — 经典蓝牙绑定协调器
│   ├── GpsData.ets                    — 共享 GPS 状态对象（@Observed 响应式）
